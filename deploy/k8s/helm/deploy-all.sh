...
if [[ $clean ]]; then
  ...
  if [[ -z $(helm ls -q --namespace $namespace) ]]; then
    ...
    helm delete $(helm ls -q --namespace $namespace)
...
charts=(eshop-common apigwwm apigwws basket-api catalog-api identity-api locations-api marketing-api ordering-api ordering-backgroundtasks ordering-signalrhub payment-api webmvc webshoppingagg webspa webstatus webhooks-api webhooks-web)

if [[ !$skip_infrastructure ]]; then
  for infra in "${infras[@]}"
  do
    echo "Installing infrastructure: $infra"
    helm install --namespace $namespace --set "ingress.hosts={$dns}" --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --name-template="$app_name-$infra" $infra     
  done  
fi
